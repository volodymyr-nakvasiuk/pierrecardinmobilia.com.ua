...
// Get the order
$order = $admin->orders->get_order(intval($admin->request->post('invoice')));
if(empty($order))
	die('Order not found');
...
if ($admin->request->post('receiver_email') != $settings['business']) 
	die("Incorrect merchant email"); 

// Verify transaction type
if ($admin->request->post('txn_type') != 'cart') 
...
{			
	$price = $admin->money->convert($purchase->price, $method->currency_id, false);
	$price = round($price, 2);
...
{
	$delivery_price = $admin->money->convert($order->delivery_price, $payment_method->currency_id, false);
	$delivery_price =round($delivery_price, 2);
	$total_price += $delivery_price;
}
if($total_price != $admin->request->post('mc_gross'))
	die("Incorrect total price (".$total_price."!=".$admin->request->post('mc_gross').")");
