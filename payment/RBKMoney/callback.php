...
if($amount != $admin->money->convert($order->total_price, $method->currency_id, false) || $amount<=0)
	die("incorrect price\n");

	
// Проверка наличия товара для RBK не актуальна, так как платёж проходит независимо от нашего ответа
/*
$purchases = $admin->orders->get_purchases($order->id);
foreach($purchases as $purchase)
{
	$variant = $admin->variants->get_variant(intval($purchase->variant_id));
