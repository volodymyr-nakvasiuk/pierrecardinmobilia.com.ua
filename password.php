...
if($c = $admin->request->get('code'))
{
	...
	if($new_password = $admin->request->post('new_password'))
	{
		// Файлы, отвечающие за авторизацию
		$htaccess_file = $admin->config->root_dir.'admin/.htaccess';
		$passwd_file = $admin->config->root_dir.'admin/.passwd';
		...
		else
		{
			...
			$new_login = $admin->request->post('new_login');
			$admin->config->set_admin_password($new_login, $new_password);
			
			// На всякий случай укажем заново путь к файлу .passwd
			$htaccess = file_get_contents($htaccess_file); 
			$htaccess = preg_replace('/AuthUserFile[\s]+[^\r\n]+/', 'AuthUserFile '.$admin->config->root_dir.'admin/.passwd', $htaccess);
			...
			print "
				...
				<p>
				<a href='".$admin->root_url."/admin'>Перейти в панель управления</a>
				</p>
			";
		}
	}
...
else
{
	print "
		...
		<form method='post' action='".$admin->root_url."/password.php'>
	...
	$admin_email = $admin->settings->admin_email;
	
	if(isset($_POST['email']))
	{
		if($_POST['email'] === $admin_email)
		{
			$code = $admin->config->token(mt_rand(1, mt_getrandmax()).mt_rand(1, mt_getrandmax()).mt_rand(1, mt_getrandmax()));
			...
			$message .= 'Для смены пароля перейдите по ссылке '.$admin->config->root_url.'/password.php?code='.$code.'<br>';
			$message .= 'Если письмо пришло вам по ошибке, проигнорируйте его.';
			
			$admin->notify->email($admin_email, 'Восстановление пароля администратора '.$admin->settings->site_name, $message, $admin->settings->notify_from_email);
		}
