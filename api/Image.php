...
class Image extends Admin
{
    ...
    {
        list($source_file, $width, $height, $set_watermark, $actual_size) = $this->get_resize_params($filename);
        ...
        if (substr($source_file, 0, 7) == 'http://')
        {
            ...
            $resized_file = $this->add_resize_params($original_file, $width, $height, $actual_size?'a':$set_watermark);
        }
        else
        {
        ...
        if (class_exists('Imagick'))
            $this->image_constrain_imagick($originals_dir . $original_file, $preview_dir . $resized_file, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $sharpen, $actual_size);
        else
            $this->image_constrain_gd($originals_dir . $original_file, $preview_dir . $resized_file, $width, $height, $watermark, $watermark_offet_x, $watermark_offet_y, $watermark_transparency, $actual_size);
    ...
    public function add_resize_params($filename, $width = 0, $height = 0, $set_watermark = false)
    {
        ...
        if ($width > 0 || $height > 0)
            $resized_filename = $file . '.' . ($width > 0 ? $width : '') . 'x' . ($height > 0 ? $height : '') . ($set_watermark ? $set_watermark : '') . '.' . $ext;
        else
            $resized_filename = $file . '.' . ($set_watermark ? ($set_watermark . '.') : '') . $ext;
    ...
    public function get_resize_params($filename)
    {
        ...
        if (!preg_match('/(.+)\.([0-9]*)x([0-9]*)(w|a)?\.([^\.]+)$/', $filename, $matches))
            return false;
        ...
        $actual_size = $matches[4] == 'a';
        $ext = $matches[5];      // расширение файла

        return array($file . '.' . $ext, $width, $height, $set_watermark, $actual_size);
    }
     ...
     * @return bool
     */
    private function image_constrain_gd($src_file, $dst_file, $max_w, $max_h, $watermark = null, $watermark_offet_x = 0, $watermark_offet_y = 0, $watermark_opacity = 1, $actual_size = false)
    {
        $quality = 100;
        ...
        @list($dst_w, $dst_h, $src_x, $src_y, $src_w, $src_h) = $this->calc_contrain_size($src_w, $src_h, $max_w, $max_h, $actual_size);
     ...
     * @return bool
     */
    private function image_constrain_imagick($src_file, $dst_file, $max_w, $max_h, $watermark = null, $watermark_offet_x = 0, $watermark_offet_y = 0, $watermark_opacity = 1, $sharpen = 0.2, $actual_size = false)
    {
        $thumb = new Imagick();
        ...
        // Уменьшаем
        $thumb->thumbnailImage($dst_w, $dst_h, $actual_size);
        ...
        {
            // Уменьшаем
            $frame->thumbnailImage($dst_w, $dst_h, $actual_size);
     ...
     * @return array(w, h)
     */
    function calc_contrain_size($src_w, $src_h, $max_w = 0, $max_h = 0, $actual_size = false)
    {
        if ($src_w == 0 || $src_h == 0)
            return false;
        ...
        if ($actual_size)
        {
            ...
            $dst_ratio = $max_w/$max_h;
            $src_ratio = $src_w/$src_h;
            if ($dst_ratio > $src_ratio) {
                $src_x = 0;
                $src_h_orig = $src_h;
                $src_h = $src_w*$dst_h/$dst_w;
                $src_y = 0.5 * ($src_h_orig - $src_h);
            }
            else {
                $src_w_orig = $src_w;
                $src_w = $src_h*$dst_w/$dst_h;
                $src_x = 0.5 * ($src_w_orig - $src_w);
                $src_y = 0;
            }
        }
        else
        ...
        return array($dst_w, $dst_h, $src_x, $src_y, $src_w, $src_h);
    }
